...
v8_pisa = pisa_df[(pisa_df["ESCS"].notna()) &
...
v8_hsls = hsls_df[(hsls_df["X1TXMQUINT"].notna()) &
                   (hsls_df["X1STUEDEXPCT"].notna()) &
                   (hsls_df["X1SESQ5"].notna()) &
                   (hsls_df["X1SEX"].isin([1, 2]))].copy()
v8_hsls = v8_hsls[(v8_hsls["X1TXMQUINT"] > 0) &
                   (v8_hsls["X1STUEDEXPCT"] > 0) &
                   (v8_hsls["X1SESQ5"] > 0)]

v8_hsls["Gender"] = v8_hsls["X1SEX"].map({1: "Male", 2: "Female"})
v8_hsls["Math_Quintile"] = v8_hsls["X1TXMQUINT"].map({
    1: "Q1 (Lowest)", 2: "Q2", 3: "Q3", 4: "Q4", 5: "Q5 (Highest)"
})
v8_hsls["SES_Quintile"] = v8_hsls["X1SESQ5"].map({
    1: "Q1 (Lowest)", 2: "Q2", 3: "Q3", 4: "Q4", 5: "Q5 (Highest)"
})
v8_hsls["Ed_Expect_Num"] = v8_hsls["X1STUEDEXPCT"]

v8_hsls_agg = v8_hsls.groupby(["Gender", "Math_Quintile", "SES_Quintile"]).agg(
...
v8_quintile_order = ["Q1 (Lowest)", "Q2", "Q3", "Q4", "Q5 (Highest)"]
v8_ses_colors = ["#440154", "#3b528b", "#21918c", "#5ec962", "#fde725"]

v8_gender_oecd_domain = ["Female OECD", "Female Non-OECD", "Male OECD", "Male Non-OECD"]
v8_gender_oecd_colors = ["#E91E63", "#F48FB1", "#1976D2", "#64B5F6"]
...
v8_right_chart = alt.Chart(v8_hsls_agg).mark_bar(clip = True).encode(
    x = alt.X("Math_Quintile:N", title = "Math Quintile", sort = v8_quintile_order,
    ...
    color = alt.Color("SES_Quintile:N", title = "SES Quintile",
                   scale = alt.Scale(domain = v8_quintile_order, range = v8_ses_colors),
                   legend = alt.Legend(orient = "top")),
    xOffset = alt.XOffset("SES_Quintile:N", sort = v8_quintile_order,
